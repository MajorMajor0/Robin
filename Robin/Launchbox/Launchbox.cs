...
namespace Robin.Launchbox
{
	class Launchbox : IDB
	{
		...
		public void CachePlatformData(Robin.Platform platform)
		{
			if (!platformsCached)
			{
		...
		public void CachePlatformGames(Robin.Platform platform)
		{
			if (launchboxFile == null)
			{
				GetLaunchBoxFile();
			...
			foreach (XElement gameElement in platformGameElements)
			{
				...
				if (lbGame.LBPlatform_ID != platform.LBPlatform.ID)
				{
					...
					Robin.Release release = R.Data.Releases.Local.FirstOrDefault(x => x.ID_LB == lbGame.ID);
					if (release != null)
					{
						release.ID_LB = null;
					}
		...
		public void CachePlatformReleases(Robin.Platform platform)
		{
		...
		public void CachePlatformImages(Robin.Platform platform)
		{
			if (launchboxFile == null)
			{
				GetLaunchBoxFile();

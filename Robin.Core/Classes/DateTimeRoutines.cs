...
namespace Robin.Core
{
    ...
    public static class DateTimeRoutines
    {
        ...
        static public bool TryParseDate(this string str, DateTimeFormat default_format, out ParsedDateTime parsed_date)
        {
            ...
            //look for dd/mm/yy
            System.Text.RegularExpressions.Match m = Regex.Match(str, @"(?<=^|[^\d])(?'day'\d{1,2})\s*(?'separator'[\\/\.])+\s*(?'month'\d{1,2})\s*\'separator'+\s*(?'year'\d{2}|\d{4})(?=$|[^\d])", RegexOptions.Compiled | RegexOptions.IgnoreCase);
            if (m.Success)
            {
                DateTime date;
		...
		public static DateTime? SafeGetDate(string DateString)
		{
			DateTimeRoutines.ParsedDateTime dt;
			if (DateString != null && DateTimeRoutines.TryParseDate(DateString, DateTimeRoutines.DateTimeFormat.USA_DATE, out dt))
			{
				return dt.DateTime;
			}
		...
		public static DateTime? SafeGetDateTime(string DateString)
		{
			DateTimeRoutines.ParsedDateTime dt;
			if (DateString != null && DateTimeRoutines.TryParseDateTime(DateString, DateTimeRoutines.DateTimeFormat.USA_DATE, out dt))
			{
				return dt.DateTime;
			}
